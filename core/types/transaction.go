...
//txdata中没有发送方地址，实际上发送方的地址是可以根据交易数据以及签名推算出来的
type txdata struct {
	...
	//3个签名字段和1个hash字段
	// Signature values签名的数值，签名被拆分成三段，分别是V、R、S
...
//把签名数据填充到交易中，生成的签名数据是字节数组类型，需要通过signer.SignatureValues()函数转换成3个big.Int类型的数据，然后填充到Transaction结构的R / S / V字段上
