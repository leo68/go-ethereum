...
//insert将新的头块注入当前块链。 该方法假设该块确实是真正的头。
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	//Td即总难度（Total Difficulty），由于以太坊要求总是选择最长（总难度最大）的链作为主链，
	// 通过比较这两个值就可以知道自己挖出来的块是有效块还是叔块。
	//这里计算出了链上当前的总难度localTd和新挖出来的区块所对应的总难度externTd。
	...
	bc.mu.Lock()
	defer bc.mu.Unlock()
	//链上当前的总难度localTd和新挖出来的区块所对应的总难度externTd
	...
	//首先判断externTd和localTd的大小，会出现3种情况：
	//externTd > localTd：说明新挖出的区块是有效块，有资格作为链头
	//externTd < localTd：说明已经有人在你之前挖出了新区块，且总难度更高，你挖出的是叔块
	//externTd = localTd：说明已经有人在你之前挖出了新区块，且总难度和你相同。这种情况应该极少，
	//如果出现的话，通过一个随机数来决策是否需要接受新挖出来的块作为链头
	...
	//如果决定接受新挖出的区块作为链头，则需要判断当前链头是否是新区块的父块，
	//如果不是的话需要进行重组，同时把状态设置为CanonStatTy。否则的话把状态设置为SideStatTy。
	if reorg {
