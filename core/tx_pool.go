...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	//先计算交易的hash值，然后判断是不是已经在txpool 中，在的话就直接退出。
	hash := tx.Hash()
	...
	//在当前txpool已满的情况下，剔除掉低油价的交易。之前有个priced字段存储了按gas price以及nonce排序的交易列表,这里会先把当前交易的gas price和当前池中的最低价进行比较：
	//如果低于最低价，直接丢弃该交易返回
	...
	//这一段是为了处理两个交易nonce相同的问题。如果用户发起了一笔交易，在还没有被执行之前又用同样的nonce发起了另一笔交易，
	//则只会保留gas price高的那一笔。这个list.Overlaps()函数就是用来判断pending列表中是否包含相同nonce的交易的。
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) {
    ...
    //先更新了最后一次心跳时间，然后更新账户的nonce值，最后一行就是发送一个TxPreEvent事件，
    // 外部可以通过SubscribeTxPreEvent()函数订阅该事件
...
func (pool *TxPool) addTx(tx *types.Transaction, local bool) error {
	...
	if !replace {
		from, _ := types.Sender(pool.signer, tx) // already validated
		//promoteExecuteables()则会从queue中选取一些交易放入pending列表中等待执行。
