...
import (
	...
	"github.com/meitu/go-ethereum/eth/fetcher"
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	//另一个订阅内存池交易的地方，启动了一个goroutine来接收TxPreEvent事件
...
//每当从远程对等方收到入站消息时，都会调用handleMsg。 返回任何错误后，远程连接将被拆除。
//经过一系列的检查和握手之后， 循环的调用了handleMsg方法来处理事件循环。 这个方法很长，
// 主要是处理接收到各种消息之后的应对措施。
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockMsg:
		...
		//将节点标记为区块所有者并安排区块导入，标记后的区块不会再传到到此peer
...
//广播块将传播一个块到它的对等体的一个子集，或者只会宣布它的可用性（取决于所请求的）。
...
func (self *ProtocolManager) minedBroadcastLoop() {
	...
	for obj := range self.minedBlockSub.Chan() {
		//获取事件数据类型，如果是新挖矿的区块则进行广播
		switch ev := obj.Data.(type) {
		case core.NewMinedBlockEvent:
			//首先向peer广播区块
			self.BroadcastBlock(ev.Block, true)  // First propagate block to peers
			//声明区块已经存在自身的区块链中，并非广播区块
