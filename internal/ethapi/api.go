...
type SendTxArgs struct {
	...
	GasPrice *hexutil.Big    `json:"gasPrice"`
	Value    *hexutil.Big    `json:"value"`
	//交易还可以携带一些额外数据，存放在data中，为了向后兼容
	Data     hexutil.Bytes   `json:"data"`
	//nonce是一个随账户交易次数自增的数字，一般会自动填充
	Nonce    *hexutil.Uint64 `json:"nonce"`
	Type     types.TxType    `json:"type"`
}
...
func (args *SendTxArgs) toTransaction() *types.Transaction {
	//如果目标地址为空的话，表示这是一个创建智能合约的交易，调用NewContractCreation()。否则说明这是一个普通交易，调用NewTransaction()。
...
//Backend参数，是在eth Service初始化时创建的.这里先调用了SendTx()函数提交交易，然后如果发现目标地址为空，表明这是一个创建智能合约的交易，会创建合约地址。
func submitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.To() == nil {
		...
		//创建合约地址，参数是发送方地址和交易的nonce值，然后调用CreateAddress()方法，
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	var chainID *big.Int
	//判断是否符合EIP155规范，chainID也就是我们的“--networkid”命令行参数
	if config := s.b.ChainConfig(); config.IsEIP155(s.b.CurrentBlock().Number()) {
		chainID = config.ChainId
	}
    //交易签名
