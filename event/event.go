...
//订阅为给定类型的事件创建订阅。订阅的频道在未订阅或关闭MUX时关闭。
...
//Post向所有为给定类型注册的接收者发送一个事件。 如果多路复用器已停止，则返回ErrMuxClosed。
...
type TypeMuxSubscription struct {
	...
	// Chan.
	//这两个是同一个通道。 它们是分开存储的，因此postC可以设置为nil而不会影响Chan的返回值。
	postMu sync.RWMutex
...
func (s *TypeMuxSubscription) deliver(event *TypeMuxEvent) {
	...
	select {
	//将交易事件发送到postC  chan<- *TypeMuxEvent中，
	// 并被eth/handle.go中的minedBroadcastLoop()函数中的range self.minedBlockSub.Chan()接收进行广播
	case s.postC <- event:
	case <-s.closing:
	}
}
