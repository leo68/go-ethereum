...
//remote_agent 提供了一套RPC接口，可以实现远程矿工进行采矿的功能。 比如我有一个矿机，
//矿机内部没有运行以太坊节点，矿机首先从remote_agent获取当前的任务，然后进行挖矿计算，
// 当挖矿完成后，提交计算结果，完成挖矿。
package miner
...
//loop方法,和agent.go里面做的工作比较类似，当接收到任务的时候，就存放在currentWork字段里面。
//如果84秒还没有完成一个工作，那么就删除这个工作， 如果10秒没有收到hashrate的报告，那么删除这个追踪。
